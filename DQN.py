...
class DQN:
    def __init__(self, params):
        ...
        with tf.name_scope("Layer_1_Convolutional_Layer"):
            layer_name = 'conv1';
            size = 3;
        ...
        with tf.name_scope("Layer_2_Convolutional_Layer"):
            layer_name = 'conv2';
            size = 3;
            channels = 16;
            filters = 32;
        ...
        with tf.name_scope("Layer_3_Fully_Connected_Layer"):
            layer_name = 'fc3';
            hiddens = 256;
        ...
        # Layer 4
        with tf.name_scope("Layer_4_Fully_Connected_Layer"):
            layer_name = 'fc4';
            hiddens = 4;
            dim = 256
        ...
        self.train_writer = tf.summary.FileWriter('./tflogs/offensive/train',
                                                  self.sess.graph)
        # self.test_writer = tf.summary.FileWriter('./tflogs/test')
    ...
    def train(self, bat_s, bat_a, bat_t, bat_n, bat_r):
        ...
        _, cnt, cost, self.summary = self.sess.run([self.rmsprop, self.global_step, self.cost, self.merged], feed_dict=feed_dict)
        # print 'Training cost %.4f' % cost
        return cnt, cost

    def save_ckpt(self, filename):
        self.saver.save(self.sess, filename)

    def write_summay(self,nstep):
        self.train_writer.add_summary(self.summary,nstep)
